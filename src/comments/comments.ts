...
export class Comments extends LitElement {
  ...
  @property({ type: Boolean }) closed = true;
  @property({ type: Array }) options = [];
  @property({ type: String }) chatroomId = "";

  @property({ type: Object }) user: User = { id: "", name: "" };

  subscribeCallback = (comment) => {
    this.comments.push(comment);
    this.comments = Array.from(new Set(this.comments.map((a) => a.id))).map(
      (id) => {
  ...
  render() {
    return html`
      ${this.comments.map((comment: Comment) => this.commentTemplate(comment))}
  ...
  async handleDeleteCommentClick(id: String) {
    await fetchData("http://localhost:3000/Comments/" + id, "PATCH", {
      ...
      .then((data) => {
        this.comments.splice(
          this.comments.findIndex((comment) => comment.id === data.id),
          1
        );
        this.requestUpdate();
      })
